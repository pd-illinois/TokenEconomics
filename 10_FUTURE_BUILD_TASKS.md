...
**Status:** In progress — urgent release gate  
...
- The release candidate is built from a clean, identified Git revision. Because the TokenEconomics workspace root is not currently a Git repository and the nested FutureTokenPredictor repository has pre-existing uncommitted changes, publication remains blocked until repository destination, ownership of those changes, and clean revision provenance are resolved without discarding work.
