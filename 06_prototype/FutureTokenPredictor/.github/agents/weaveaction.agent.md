...
> **Start now (Copilot CLI app).** Selecting this agent with `/agent weaveaction`
...
> agent needs a block id to build — if none was given, ask exactly one question to
> get it, then proceed.
...
10. **Hand off.** Point to the next ready block, or to `copilot @weaverevert` if
    the block needs to be rolled back.
