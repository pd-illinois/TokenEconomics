...
> **Start now (Copilot CLI app).** Selecting this agent with `/agent weavemvp`
...
1. **Load the idea and the lessons.** Read `product.idea` from
   ...
   `copilot @weaveidea`. Then read `.weave/lessons.yaml` if it exists: each entry
   is a block that was rolled back and *why*. Carry those reasons into the cut --
...
9. **Hand off.** Point the user to `copilot @weaveaction` to build the first
   block whose dependencies are already `done`.
