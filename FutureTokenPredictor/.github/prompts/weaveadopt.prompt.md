...
WITHOUT rewriting it, so the build loop (`/weavemvp`, `/weaveaction`,
`/weaverevert`) can take over from a truthful starting point.
...
1. **Fingerprint the repo.** Run `python scripts/detect_project.py`. It reports
   ...
   stop and point the user to `/weaveidea` instead -- there is nothing to adopt.
...
6. **Hand off.** Show the rendered `kanban.md`: baseline vs unverified at a
   glance. Point the user to `/weavemvp` to plan new work on top of the adopted
   foundation, or to keep characterizing unverified seams until they are trusted.
